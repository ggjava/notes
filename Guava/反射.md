...
注意如果`mapToken`只是返回了`new TypeToken<Map<K, V>>()`，它实际上不能把具体化的类型分配到 K 和 V 上面，举个例子
...
`TypeToken`将Java提供的`TypeVariables`和`context token`中的类型变量统一起来。这可以被用来一般性地推断出在一个类型相关方法的返回类型：
